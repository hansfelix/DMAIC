    ...
    <v-container fluid grid-list-lg>
      <v-layout row align-center wrap>
        ...
        <v-flex xs12 md8 align-center>
          ...
          <span>Se listan a los ususarios resgistrados, para añadir uno nuevo, de click en el botón
            <strong>AÑADIR USUARIO</strong>:</span>
          <v-spacer></v-spacer>
        </v-flex>
      ...
      <v-layout row align-center wrap>
        <v-flex xs12>
          ...
          <v-data-table :headers="mainHeaders" :items="Users" :search="search" item-key="nombre" hide-actions class="elevation-1">
            ...
            <div slot="no-results" :value="true" color="error">
              Tu búsqueda "{{ search }}" no obtuvo resultados.
            </div>
          </v-data-table>
        </v-flex>
      </v-layout>
                ...
                <v-card-text>
                  <v-container grid-list-md>
                    <v-layout wrap>
                      ...
                      <v-flex xs12>
                        <v-text-field label="Contraseña" id="txt_userContrasena" v-model="txt_userContrasena" hint="Use una contraseña conveniente"
                          required></v-text-field>
                      </v-flex>
                      <v-flex xs12>
                        <v-switch :label="`Modo adminstrador: ${(switchAdministrador)? 'activado': ''}`" v-model="switchAdministrador"></v-switch>
                      </v-flex>
                      <v-flex xs12>
