...
<v-layout class="text-xs-center background_navigationDrawer" align-end justify-end>
     <!-- <img src="@/assets/background_navigationDrawer.jpg" class="img_background_navigationDrawer" top/> -->
         <v-toolbar flat class="transparent" align-end justify-end>
          <v-list class="pa-0 transparent " align-end justify-end >
            <v-list-tile avatar>
              ...
              <v-list-tile-content>
                <h4>{{user.nombre}}</h4>
                <v-list-tile-title class="navDrawer_email"><small>{{user.correo}}</small></v-list-tile-title>
              </v-list-tile-content>
            </v-list-tile>
          </v-list>
        </v-toolbar>
      ...
      <v-list>
        <v-list-tile 
          value="true"
          v-for="(item, i) in itemsAdminstrador"
          :key="i"
          v-bind:to="item.url"
        >
          <v-list-tile-action>
    ...
    <v-footer :fixed="fixed" app>
      <span>&copy; COSAPI 2018 <small>- DERECHOS RESERVADOS</small> </span>
    </v-footer>
  </v-app>
</template>
  ...
  computed: {
    ...
    itemsAdminstrador() {
      ...
      if (this.user.administrador) {
        menuItems = [
          { icon: "supervisor_account", title: "Usuarios", url: "/Usuarios" },
          {
            icon: "settings",
            title: "Configuración",
            url: "/Configuracion"
